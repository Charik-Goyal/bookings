...
// GetUserByID returns use by ID
func (m *postgresDBRepo) GetUserByID(id int) (models.User, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	query := `select id, first_name, last_name, email, password, access_level, created_at, updated_at from user 
			  where id=$1`
...
// GetReservationByID return reservation by ID
...
// UpdateProcessForReservation updates processed from a reservation by id
...
func (m *postgresDBRepo) GetRestrictionsForRoomByDate(roomID int, start, end time.Time) ([]models.RoomRestriction, error) {
	...
	// coalesce check if null then give 0 otherwise give the same evalue
	query := `
